...
public class RemoteLogcatServer implements Runnable {
     ...
     */
    private byte[] readLogcatContent(String route, boolean previousCleaning) {
        ...
        try {
            ...
            if (previousCleaning) {
                if( //Only for Rooted devices => result=0
                    ...
                    Log.d("CLEAN_LOG_FLAG", CLEAR_LOG_TAG + Calendar.getInstance().getTimeInMillis());
                    lastCleanFlag = CLEAR_LOG_TAG + Calendar.getInstance().getTimeInMillis();
                }
            }
    ...
    private byte[] getBytesFromString(String inputString) throws IOException {
        ...
        InputStream inputData = new ByteArrayInputStream(inputString.getBytes(StandardCharsets.UTF_8));
